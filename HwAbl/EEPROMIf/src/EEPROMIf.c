...
uint8_t EEPROMIf_HwSerial_au8[EEPROM_LEN_HW_SERIAL];
...
static void EEPROMIf_Read(uint16_t Addr_u16, uint8_t* Data_pu8, uint16_t Len_u16)
{
    uint16_t i_u16;
...
void EEPROMIf_Init(void)
{
    ...
    EEPROMIf_Read(EEPROM_ADDR_HW_SERIAL, EEPROMIf_HwSerial_au8, EEPROM_LEN_HW_SERIAL);
    
}

uint8_t EEPROMIf_GetHwRevision(void)
{
    return EEPROMIf_HwRev_u8;
}

const uint8_t* EEPROMIf_GetHwSerial(void)
{
    return EEPROMIf_HwSerial_au8;
}
